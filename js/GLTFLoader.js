...
THREE.GLTFLoader = ( function () {
	...
	function addMorphTargets( geometry, targets, parser ) {
		...
		for ( let i = 0, il = targets.length; i < il; i ++ ) {
			...
			if ( hasMorphPosition ) {
				...
				let accessor = target.POSITION !== undefined
			...
			if ( hasMorphNormal ) {
				...
				// TODO: Error-prone use of a callback inside a loop.
				let accessor = target.NORMAL !== undefined
	...
	function isPrimitiveEqual( a, b ) {
		...
		let dracoExtA = a.extensions ? a.extensions[ EXTENSIONS.KHR_DRACO_MESH_COMPRESSION ] : undefined;
		let dracoExtB = b.extensions ? b.extensions[ EXTENSIONS.KHR_DRACO_MESH_COMPRESSION ] : undefined;
				...
				for ( let i = 0, il = geometries.length; i < il; i ++ ) {
					...
					for ( let j = 0, jl = materials.length; j < jl; j ++ ) {
						...
						if ( mesh.isPoints ) {
							...
							let pointsMaterial = parser.cache.get( cacheKey );
							...
							let lineMaterial = parser.cache.get( cacheKey );
						...
						if ( useVertexColors || useFlatShading || useSkinning || useMorphTargets ) {
							...
							let cachedMaterial = parser.cache.get( cacheKey );
